...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Config defines GPIO1/GPIO0 inputs and safe WS2812 LED candidate pins GPIO8/GPIO20/GPIO21.
...
The MicroPython ESC simulator app skeleton is in place. The reversible PoC `boot.py` executes `main.py`, `main.py` starts `cyberbrick_esc.app.main()`, and the app initializes status LED feedback, PWM input capture, and safety mapping before running the 200 Hz control loop. Configuration centralizes GPIO1/GPIO0 inputs, safe LED candidate pins GPIO8/GPIO20/GPIO21, pulse timing defaults, failsafe timing, neutral arming, and loop rate. Host tests now verify the default skeleton configuration and that reserved motor pins GPIO4-GPIO7 are not configured or directly referenced by MicroPython source.
